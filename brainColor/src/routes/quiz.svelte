<script>
  import { goto } from '$app/navigation'; // 화면 이동을 위한 goto 함수 선언
  import { score, quizList } from '$lib/store'; // 스토어 변수 사용 선언
  ...
  function next() {
    ...
    if (idx == list.length) {
      ...
      // 문제가 남아 있으면
      current = list[idx]; // list에서 새로운 문제를 꺼냄
  ...
  function resetProgress() {
    ...
    bonus = 100; // 보너스 100 점으로 시작
    timer = setInterval(function () {
      bonus = bonus - 1; // 보너스를 1점씩 감소시킴
      if (bonus == 0) {
        // 보너스 점수가 0이 되면
        clearInterval(timer); // 타이버 초기화
      }
    }, 40); // 1점 감소 주기는 40밀리 초
  }
